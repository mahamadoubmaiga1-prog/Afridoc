...
function pdfHeader(doc, title, theme) {
  ...
  if (theme.name === 'minimaliste') {
    doc.rect(36, 20, pageWidth - 72, 74).lineWidth(1).strokeColor(theme.borderColor).stroke();
    doc.fillColor(theme.headerText).fontSize(28).font('Helvetica-Bold').text('AFRIDOC', 48, 32, { align: 'center' });
    doc.fontSize(11).font('Helvetica').text('Génération Professionnelle de Documents Administratifs', 48, 62, { align: 'center' });
  } else {
    doc.rect(0, 0, pageWidth, 100).fill(theme.headerBg);
    doc.rect(0, 95, pageWidth, 5).fill(theme.accentColor);
    doc.fillColor(theme.accentColor).fontSize(28).font('Helvetica-Bold').text('AFRIDOC', 48, 18, { align: 'center' });
    doc.fillColor(theme.headerText).fontSize(11).font('Helvetica').text('Génération Professionnelle de Documents Administratifs', 48, 52, { align: 'center' });
  }

  doc.moveDown(theme.name === 'minimaliste' ? 2.2 : 2.5);
  doc.fillColor(theme.labelColor).fontSize(16).font('Helvetica-Bold').text(title, { align: 'center' });
  doc.moveTo(48, doc.y + 8).lineTo(pageWidth - 48, doc.y + 8).strokeColor(theme.accentColor).lineWidth(theme.name === 'minimaliste' ? 1 : 2).stroke();
  doc.moveDown(1.5);
...
function pdfFooter(doc, theme) {
  ...
  doc.moveTo(48, bottom).lineTo(doc.page.width - 48, bottom).strokeColor(theme.accentColor).lineWidth(1).stroke();
  doc.fillColor(theme.mutedColor).fontSize(9).font('Helvetica').text('Généré par Afridoc — Document Administratif Professionnel', 48, bottom + 10, { align: 'center' });
  doc.fillColor(theme.mutedColor).fontSize(8).text(`Generated on ${new Date().toLocaleDateString('fr-FR')}`, 48, bottom + 22, { align: 'center' });
}
